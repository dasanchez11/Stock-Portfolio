...
const Modal = () => {
    const tickerName = useAppSelector(state => state.mainContent.stockEdit)
    const data = useAppSelector(state=>state.portfolio.data)
    const dataResult = data.filter(item=> item.ticker === tickerName)
    
    
    const dispatch = useAppDispatch()
    const [changeInfo,setChangeInfo] = useState<ChangeInfo>({id:{numOfShares:'',entryPrice:''}})

   

    const handleClickDelete = (e:React.MouseEvent<HTMLButtonElement>) =>{
        e.preventDefault()
        const id = (e.target as HTMLElement).closest("Button")?.getAttribute('id')
        if(id){
    ...
    const handleClickUpdate = (e:React.MouseEvent<HTMLButtonElement>) =>{
        e.preventDefault()
        const id = (e.target as HTMLElement).closest("Button")?.getAttribute('id')
        if(id){
                ...
                {dataResult.map(item=>{
                    return(
                        <form key={item._id} id={item._id} className='relative text-white border border-green-600 p-4 rounded-md w-[80%] md:w-[60%] flex flex-col items-center'>
                            ...
                            <input id={item._id} autoComplete='off' type='number' name='entryPrice' onChange={handleChange} className='w-[80%] text-xs sm:text-base border-b-4 focus:text-white px-4 border-green-600 outline-none text-green-600 placeholder:bg-gray-500 placeholder:text-white bg-gray-500' placeholder={String(item.entryPrice)}></input>
                            <div className='w-[80%] text-xs sm:text-base text-center'>Number of Shares</div>
                            <input id={item._id} autoComplete='off' type='number' name='numOfShares' onChange={handleChange} className='w-[80%] text-xs sm:text-base border-b-4 focus:text-white px-4 border-green-600 outline-none text-green-600 placeholder:bg-gray-500 placeholder:text-white bg-gray-500' placeholder={String(item.numOfShares)}></input>
