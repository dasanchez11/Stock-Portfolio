...
const StockCard = ({data}) => {
  ...
  let graphData = useSelector(state=>state.portfolio.histData[ticker].values)
  graphData = prepareDataToGraph(graphData)
  const trend = graphDataTrend(graphData)


  const handleClickView = (e) =>{
    const val = e.target.closest("DIV").getAttribute('name')
  ...
  return (
    <div name={ticker.toLowerCase()} className='relative flex flex-row h-20 gap-1 justify-center mx-8 sm:mx-10 lg:m-0 lg:justify-between p-2 items-center bg-neutral-800 rounded-lg'>
            ...
            <div className='mt-2 w-[20%] flex flex-row items-center justify-center'>
              {type.toLowerCase() === 'crypto' ? <Icon name={name_alpha.toLowerCase()}  height="25"/> :
