...
const AddStock = () => {
    ...
    const {symbol,type,name_alpha} = useSelector(state=>state.mainContent.stockAddInfo)
    const name = useSelector(state=>state.mainContent.stockAdd)
        ...
        <div className='relative h-[70vh] w-[60vw] bg-neutral-800/90 rounded-lg flex flex-col items-center'>
            ...
            <form className='relative text-white border border-green-600 p-4 rounded-md w-[80%] md:w-[60%] flex flex-col items-center'>
                <div className='cursor-pointer absolute top-0 left-0 flex flex-row items-center text-xs'>
                    <input autoComplete='off' className='m-3 cursor-pointer'  type="checkbox" checked={currentPrice ? 'Checker':''} onChange={()=>setCurrentPrice(!currentPrice)}></input>
                    <h4>Current Price</h4>
                </div>
